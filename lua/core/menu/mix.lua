...
m.key = function(n,z)
  if n==2 and z==1 and m.sel > 1 then
    m.sel = m.sel - 1
  elseif n==3 and z==1 and m.sel < 2 then
    m.sel = m.sel + 1
  end
end

m.enc = function(n,d)
  local ch1 = {"input_level","tape_level"  }
  local ch2 = {"engine_level","monitor_level"}
  local ch3 = {"softcut_level","output_level"}
  if n==2 then
...
--Marker Lines
  for i=1,6 do
    ...
    if (i==1 and m.sel==1) or (i==4 and m.sel==2) then
      screen.rgblevel(15,4,0)
    end
    if (i==2 and m.sel==1) or (i==5 and m.sel==2) then
      screen.rgblevel(0,15,9)
    end

    if (i==3 and m.sel==1) or (i==6 and m.sel==2) then
      screen.rgblevel(2,4,15)
    end
